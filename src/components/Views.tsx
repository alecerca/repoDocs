...
export function DecisionsView() {
  ...
  const [sub, setSub] = useState<'list' | 'graph'>('list')
  const [graphSel, setGraphSel] = useState<string>(() => activeProject)

  const groups = useMemo(() => Object.entries(adrsByProject()).sort((a, b) => a[0].localeCompare(b[0])), [])
  const supMap = useMemo(() => supersededByMap(ADRS_ALL), [])
  const withAdrs = useMemo(() => groups.map(([p]) => p), [groups])
  const graphProj = withAdrs.includes(activeProject)
    ? activeProject
    : withAdrs.includes(graphSel)
      ? graphSel
      : (withAdrs[0] ?? '')

  if (groups.length === 0) {
    return <EmptyState>{t('adr.empty')}</EmptyState>
  }
